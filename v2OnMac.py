...
while gameOn:
    ...
    while restingState1:
        ...
        if sec == restingStateDurationTest :
            ...
            for chunk in range(restingStateDurationTest):
        ...
        elif sec < restingStateDurationTest:
            try:
                # queue.queue.clear()
    ...
    while fly:
        ...
        if durationSession > 0:
            try:
                ...
                # We need to wait for the buffer to be big enough for a fft, wait 800 points : 4 * 200, equivalent to 1 second
                #  and wait that the 4 channels of the packet are obtained

                while len(bufferF) < 800:
                    print len(bufferF)
                ...
                if (len(bufferF) % 4 != 0) :
                    ...
                    # TODO check that time is not a problem and that the following operations dont take too long
                    # the following operations are : slicing, filtering, extraction of alpha and delta
                    bufferF_array = np.asarray(bufferF)
                    # screen.blit(plane, (5. * w_display / 12, veryoldPosy + 1.*(oldPosy - veryoldPosy)/steps ))
                    # print bufferF_array
                    screen.blit(sky, (0,0))
                    screen.blit(plane, (5. * w_display / 12, oldPosy ))
                    # displayNumber(math.floor(scoreF), screen, 'down')
                    # displayNumber(durationSession, screen, 'down_left')
                    ...
                    dataF[0, :] = bufferF_array[-800:len(bufferF_array):4]
                    # dataF[1, :] = bufferF_array[ind_channel_2]
                    dataF[1, :] = bufferF_array[-799:len(bufferF_array):4]
                    # dataF[2, :] = bufferF_array[ind_channel_3]
                    dataF[2, :] = bufferF_array[-798:len(bufferF_array):4]
                    # dataF[3, :] = bufferF_array[ind_channel_4]
                    dataF[3, :] = bufferF_array[797:len(bufferF_array):4]
                    ...
                    if medRatioF == maxRatioAlphaOverDelta:
                        ...
                        newPosy = maxDisplayY
                        print newPosy
                    elif minRatioAlphaOverDelta - maxRatioAlphaOverDelta != 0 :
                        a = (maxDisplayY - minDisplayY) * 1. / (minRatioAlphaOverDelta - maxRatioAlphaOverDelta)
                        b = maxDisplayY - minRatioAlphaOverDelta * a
                        # TODO minRatioAlphaOverDelta and maxRatioAlphaOverDelta
                        print minRatioAlphaOverDelta, maxRatioAlphaOverDelta, a, b ,medRatioF
                        newPosy = a * medRatioF + b
                        print newPosy
                    elif minRatioAlphaOverDelta == maxRatioAlphaOverDelta:
                        newPosy = 1. * maxDisplayY / 2
